...
2. Run `notebooks/04_gold/gold_marts.ipynb` with the default catalog/schema widgets, then run `notebooks/04_quality/gold_quality.ipynb`. The Gold notebook first verifies source grains, keys, timezone and joins. It creates the single-fact star as views: `fact_taxi_trip`, `dim_date`, `dim_hour`, `dim_zone`, `dim_weather_hour`, plus derived daily output `vw_mobility_daily`. Taxi has one row per selected Silver trip; the weather dimension has one observation per local hour, linked via `fact_taxi_trip.weather_hour = dim_weather_hour.observation_hour`. On a successful Gold run, the obsolete `fact_weather_hourly` **view** from the earlier model is retired. The Gold quality notebook checks key uniqueness, dimensional and weather joins, no row multiplication, and Silver-to-Gold counts.
...
## Distance quality and verified run

`gold_marts.ipynb` exposes `max_valid_distance_miles` (default **100**, a provisional analytical rule in miles). Gold retains the original `trip_distance` and all trip rows, and marks values above the limit with `is_distance_outlier`. The daily view and all three analytics notebooks omit flagged values only from distance sums and averages; demand counts, duration, and fare measures still include those trips. A distance average divides by unflagged trips with a distance. Changing the widget requires rerunning Gold before downstream quality and analytics. The limit needs business review before treating long trips as invalid.

In the verified March–May 2026 Databricks run, Gold quality passed: 133,355 distinct Taxi fact keys, 2,208 unique Weather hours, 265 unique Zones, 93 Date rows, 24 Hour rows, and zero missing dimension/Weather joins or multiplied Taxi rows. It reported **31 flagged distances of 133,355 trips**. Q1's May 11, 08:00, East Harlem North group kept 56 trips, flagged one extreme distance, and reported 95.64 miles total / 1.74 miles average; the original 33,518.65-mile value remains available in the fact. Q2 reconciled 112,975 dry plus 20,380 wet trips to 133,355, and 28 plus 3 flagged distances to 31. Q3 displayed separate pickup/dropoff roles; weather and day/hour in both roles refer to pickup time. Dry/wet totals have unequal hour exposure, and weather represents one observation location; these comparisons do not establish causation.

## Current scope

Source inspections, Bronze loads, Silver views, quality checks, the one-fact/four-dimension Gold views, and all three business-question analytics notebooks are in Git. Gold, Gold quality, and the three analytics notebooks were run and reviewed in Databricks for the current March–May 2026 data. The notebooks have not been independently exercised against future source batches. A scheduled end-to-end job and a real Bronze ingestion timestamp/source-file audit trail remain outstanding.

### Rerun after updating `dev`

Pull `dev` into the Databricks Git folder, confirm the three Silver views, then run `notebooks/04_gold/gold_marts.ipynb`, `notebooks/04_quality/gold_quality.ipynb`, and the three `notebooks/05_analytics/` SQL notebooks in that order. Review Gold quality's grain, join, and flagged-distance counts before sharing analytics.
