...
Pull `dev` in the Databricks Git folder, confirm the three Silver views, then run `notebooks/04_gold/gold_marts.ipynb`, `notebooks/04_quality/gold_quality.ipynb`, and the three `notebooks/05_analytics/` SQL notebooks in that order. Check Gold quality before using analytics.
