...
Source inspections, Bronze loads, Silver views, quality checks, Gold views and the three analytics notebooks are in Git. The prior two-fact Gold model passed runtime verification in Databricks. This single-fact revision still needs a Databricks run, Gold quality rerun, and analytics checks; a scheduled end-to-end job is also outstanding.

### Rerun after updating `dev`

1. In the Databricks Git folder, pull the latest `dev`. Confirm that all three Silver views still show the expected March–May data.
2. Run `notebooks/04_gold/gold_marts.ipynb` (all cells). Expect 133,355 Taxi rows, 2,208 weather hours and 265 zones if Silver is unchanged. This updates the views and removes the old `fact_weather_hourly` view.
3. Run `notebooks/04_quality/gold_quality.ipynb` (all cells) and require `Single-fact Gold grain, reconciliation and joins: PASS` before analytics.
4. Run the three SQL notebooks in `notebooks/05_analytics/`. Investigate extreme trip-distance values before using distance averages to describe typical trips: an earlier Q1 result for 2026-05-11 at 08:00 in East Harlem North had 56 trips and 33,614.29 total miles, which is anomalous.
