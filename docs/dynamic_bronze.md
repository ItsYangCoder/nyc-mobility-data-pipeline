...
The unscheduled dev Job in `databricks.yml` runs Bronze, Silver, quality, and Gold as eight visible notebook tasks. Bronze passes discovered months to Silver using task values. `notebooks/00_pipeline/run_pipeline.py` remains available until the Job passes a full dev run. The Job has no production target or schedule; deployment and execution require Databricks credentials.
