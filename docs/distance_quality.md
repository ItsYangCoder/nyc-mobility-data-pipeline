...
**Policy.** `gold_marts.ipynb` defines `max_valid_distance_miles` (default: 100 miles). The Gold fact keeps every trip and its original `trip_distance`; `is_distance_outlier` flags values above the limit. The daily view and Q1–Q3 analytics omit flagged distances from distance sums and averages only. Trip counts, duration and fare measures still include those trips. The threshold is a provisional analytics choice, not a source correction. After changing it, rerun Gold, Gold quality, then analytics.
